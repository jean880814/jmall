...
#注意事项 
  * 启动类上需要扫描该包下的组件,添加@ComponentScan(basePackages = {"com.jmall.commons.lock"})
...
#关于扩展点(伪扩展点) lockType
  ...
  * 若需要扩展  1 实现DistributedLock接口 2在指定资源目录(META-INF/services/ 或者 META-INF/lock/
    或者META-INF/lock/internal/)下添加key=value (可以参考 jmall-commons\commons-lock\src\main\resources\META-INF\lock\DistributedLock)
